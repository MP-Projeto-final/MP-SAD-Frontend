...
import jsQR from 'jsqr'; // Biblioteca para leitura de QR codes
...
export default function QRCodeUpload() {
  ...
  // Função para ler QR code da imagem
  const readQRCode = async () => {
    ...
    image.onload = () => {
      ...
      if (code) {
        ...
        // Verifica se o QR code contém um texto no formato "Doacao ID: 57"
        const donationIdMatch = code.data.match(/Doacao ID: (\d+)/);
        if (donationIdMatch) {
          const donationId = donationIdMatch[1];
          navigate(`/donation/${donationId}`); // Redireciona para a página da doação
        } else {
      ...
      <PageContainer>
        ...
        {previewUrl && (
          <PreviewContainer>
            ...
            <Button onClick={readQRCode}>Ler QR Code</Button> {/* Botão para ler QR Code */}
          </PreviewContainer>
        )}
      </PageContainer>
    </>
